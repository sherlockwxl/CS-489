...
def regression(x,y):
    ...
    for i in range(0,11):
        ...
        for startpoint in range(0,10):
            ...
            errorvalidtest += numpy.linalg.norm(numpy.dot(testsetx,w)-testsety)**2/len(testsetx)
            errortrainingset += numpy.linalg.norm(numpy.dot(trainsetx,w)-trainsety)**2/len(trainsetx)


        left = numpy.dot(numpy.transpose(trainx), trainx) + (i * 10) * 1 * numpy.identity(
                numpy.transpose(trainx).shape[0])
        ...
        print ("on $\lambda$ = %d, training set error: %f, valid set mean error %f, test set error: %f, percentage of nonzeros in w %f \\\\"%(i*10, errortrainingset/10,  errorvalidtest/10, errortestset, wnonzerototal/wtotal))
