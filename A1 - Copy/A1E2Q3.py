...
def regression(x,y):
    ...
    for i in range(0,11):
        ...
        for startpoint in range(0,10):
            ...
            nonzerocount += numpy.count_nonzero(w[13:])/(1000)
        ...
        print ("on $\lambda$ = %d ,training set error: %f,validation set mean error %f, test set error: %f,  percentage of nonzeros in w %f \\\\"%(i*10,  errortrainingset/10, errorvalidtest/10, errortestset, nonzerocount/10))
